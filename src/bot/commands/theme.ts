...
async function handleDailyThemeCommand(interaction: ChatInputCommandInteraction) {
  try {
    ...
    try {
      await interaction.reply({
        content: "Failed to show theme status.",
        flags: MessageFlags.Ephemeral,
      });
    } catch {}
  }
...
async function handleLaunchDailyThemeModal(interaction: ButtonInteraction) {
  try {
    ...
    const titleLabel = new LabelBuilder().setLabel("Daily theme title").setTextInputComponent(titleInput);
    ...
    const descLabel = new LabelBuilder().setLabel("More details (optional)").setTextInputComponent(descInput);
    ...
    try {
      await interaction.reply({
        content: "Failed to open theme form.",
        flags: MessageFlags.Ephemeral,
      });
    } catch {}
  }
...
function buildSavedThemeEmbed(title: string, desc: string) {
  const embed = new EmbedBuilder()
    ...
    .setColor(0x0099ff)
    .addFields({ name: "Title", value: title }, { name: "Description", value: desc || "(none)" });
  const updateButton = new ButtonBuilder()
    .setCustomId("daily-theme-update")
    .setLabel("Update Theme")
    .setStyle(ButtonStyle.Primary);
  const clearButton = new ButtonBuilder()
    .setCustomId("daily-theme-clear")
    .setLabel("Clear Theme")
